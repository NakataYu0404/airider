...
void AnimationController::Add(int type, const std::string& path, float speed,int animIndex)
{
	...
	//	AnimIndexが指定されてたら、そのIndexを使うよ(これで1個のモデルに何個もアニメーションが設定されていた場合に使えるよ)
	if (animIndex != -1)
	{
	...
	float startStep, float endStep, bool isStop, bool isForce)
{
	...
	if (playType_ != type || isForce) {
		...
		//	アニメーションが2つか(大体MixamoのTポーズとアニメーション1個を持っているモデル)
		if (MV1GetAnimNum(playAnim_.model) == 2)
		{	
			animIdx = 1;
		}
		//	いっぱいか(Mixamo以外の自分でたくさんのアニメーションを持っているモデル)
		else if (MV1GetAnimNum(playAnim_.model) > 1)
		{
			animIdx = animations_[type].animIndex;
		}
